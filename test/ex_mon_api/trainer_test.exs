defmodule ExMonApi.TrainerTest do
  ...
  describe "changeset/1" do
    test "when all params are valid, returns a valid changeset" do
      ...
      assert %Ecto.Changeset{changes: %{name: "Jhonatan", password: "123456"}, errors: [], data: %ExMonApi.Trainer{}, valid?: true} = response
    end
    ...
    test "when there are invalid params returns invalid changeset" do
      ...
      assert %Ecto.Changeset{changes: %{password: "123456"}, data: %ExMonApi.Trainer{}, valid?: false} = response
  ...
  describe "build/1" do
    test "when all params are valid, returns a trainer struct" do
      ...
      assert {:ok, %Trainer{name: "Jhonatan", password: "123456"}} = response
    end
